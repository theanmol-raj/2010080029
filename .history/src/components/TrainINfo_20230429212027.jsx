...
function TrainINfo({data,setter}) {
  return (
    <div className=" flex m-4 rounded-3xl justify-center items-center h-screen bg-gradient-to-br from-purple-500 to-yellow-500">
      {" "}
        ...
        <div class="relative  w-full">
          <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            {data.map((item ,index) =>  <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={index}>
                ...
                >
                  <a href={`/${index}`}><button onClick={()=>setter(index)}>{item.trainName}</button></a>
                </th>
                <td class="px-6 py-4"><a href={`/${index}`}><button onClick={()=>setter(index)}>{item.trainNumber}</button></a></td>
